...
### P5 추가에서 답이 정해진 것 (N=3)

- **패널형 setMiniBox 처리**: 별도 함수 없이 **매핑 객체 흡수** (`FILLED_ID.io = 'mini-io-ports'`, `BG_BASE_CLASS.io = 'io-panel-bg'`). 슬롯형과 같은 setMiniBox 호출. 차이점은 filled element가 단일 모듈 X 여러 포트 그룹.
- **미니맵 공간 결정 — 2층 배치 (높이 확장)**: viewBox 가로 확장 vs 부품 크기 축소 vs 2층 중 **2층 채택**. viewBox 880×245 (높이만 +100), 가로 그대로. 이유: 메인보드 사실성 ↑ + 모든 부품 크기 유지 + max-width CSS 변경 X
- **트레이스 다중화**: 위층 부품 잇기(3가닥) + 위→아래(1가닥) + 아래층 부품 잇기(2가닥) = 6가닥. setMiniBus의 트레이스 배열에 추가만 하면 됨
- **페이즈 인디케이터 5단**: 한 줄 충분. 압축 명명 불필요 (각 pill 폭이 적정). 도메인 완결되면 잠김 pill 제거 (P6+ 시그널 X)
- **포트 멀티 시각**: I/O 패널의 포트들(USB·RJ45/NIC·HDMI·USB-C·AUDIO)을 각각 다른 형태로 표현 → 시각 구분 빠름. RJ45는 NIC라서 강조색(#7ee0c8)
- **도메인 완결 처리**: 5대 부품으로 컴퓨터구조 도메인 마무리. P6+ slot 제거 + 페이즈 인디케이터 잠김 pill 제거 = "이 도메인은 완성됨" 시그널
...
- `빌드업.html` — P1 CPU + P2 RAM + P3 System Bus + P4 Storage + P5 IO Device (컴퓨터구조 5대 부품 완성)
