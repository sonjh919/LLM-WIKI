...
updated: 2026-05-03 (빌드업 P5 IO Device 추가 — 컴퓨터구조 5대 부품 완성)
---
...
## [2026-05-03] output | 빌드업 P5 IO Device 추가 — 컴퓨터구조 5대 부품 완성
- 메인 무대 신규 그룹: transition4-state, p5-step0~3
  - Q1: I/O DEVICE 키워드 + 내부↔외부 경계 비유
  - Q2: 외부 단절 시각 (사용자 입력·화면 출력·네트워크 모두 불가)
  - Q3: 3분류(입력·출력·입출력) + NIC 강조 + I/O 멀티플렉싱(epoll·Selector) + Netty/WebFlux 안내
- 미니맵 **2층 배치로 재구성**: viewBox 880×145 → 880×245 (높이 +100)
  - 위층: CPU 소켓 + DIMM 슬롯 (기존 위치 유지)
  - 아래층: M.2 SSD(좌) + I/O 패널(중) + (P6+ 자리는 추후 제거)
  - System Bus 트레이스 6가닥: 위층 3 + 위→아래 1 + 아래층 2
- I/O 패널 디자인: USB-A×2, RJ45/NIC(강조), HDMI, USB-C, 3.5mm 오디오. 패널형 부품 — 패널 박스 + 여러 포트 미니어처 그룹
- **패널형 setMiniBox**: 매핑 객체 흡수로 처리 (FILLED_ID·BG_BASE_CLASS에 io 키 추가). 별도 함수 안 만듦. 슬롯형과 동일하게 setMiniBox 호출
- computeView·prevMinimapState·jumpToPhase·페이즈 인디케이터·updatePhasePills·nextBtn·render·showCompletionUI 모두 P5 흐름 확장
- transition4 = "외부와 단절" 메시지 (사람·네트워크 이모지로 외부 세계 표현)
- 도메인 완결 처리: P6+ slot 제거 + 페이즈 인디케이터 잠김 pill 제거 = 컴퓨터구조 도메인 완성 시그널

## [2026-05-03] reflect | 빌드업 가이드 CLAUDE.md에 P5 학습 반영
- "P5+에서 도달 시 답을 정할 것" 섹션 제거 (모두 답 정해짐)
- "P5 추가에서 답이 정해진 것 (N=3)" 섹션 신규:
  - 패널형 setMiniBox = 매핑 객체 흡수 (별도 함수 X)
  - 미니맵 공간 결정 = 2층 배치 (높이 확장, 가로 그대로)
  - 트레이스 다중화 (6가닥)
  - 페이즈 인디케이터 5단 한 줄 충분, 도메인 완결 시 잠김 pill 제거
  - 포트 멀티 시각 (각 포트마다 다른 형태)
- "현재 파일": **컴퓨터구조 5대 부품 완성** (P1~P5)
- 빌드업 형태 자체의 일반성 N=3(P3·P4·P5에서 패턴 검증) 시점 — 다음 사이클은 다른 도메인 적용 또는 스킬화 검토

## [2026-04-30] output | 빌드업 P4 Storage 추가 (슬롯형 추상화 검증)
- 메인 무대 신규 그룹: transition3-state, p4-step0~3
  - Q1: STORAGE / 보조기억장치 + RAM(작업공간) vs Storage(창고) 비유
  - Q2: RAM 한계 (전원 OFF → 데이터 사라짐) + 필요한 것 (비휘발성·대용량)
  - Q3: 시스템 흐름(Storage → RAM → CPU) + HDD vs SSD 비교
- 미니맵 M.2 슬롯 추가: 기존 P4+ 빈 슬롯 자리(x=660~830)에 M.2 슬롯 + 마운트 볼트. 학습 완료 시 SSD 모듈 등장(검은 PCB + NAND 칩 3개 + "SSD" 라벨)
- 슬롯형 setMiniBox 추상화: `FILLED_ID`, `BG_BASE_CLASS` 매핑 객체로 일반화 — 새 슬롯형 부품 추가 시 키만 추가하면 됨
- computeView에 transition3·p4·complete 분기. view에 storage 필드
- prevMinimapState storage 키, jumpToPhase '4' 분기, 페이즈 인디케이터 P4 pill 추가, P4+ → P5+ 라벨
- updatePhasePills 8단 order 배열로 확장
- 메인 무대 시각 미세 조정: Storage→RAM→CPU 화살표 방향 보정(박스 외곽 모서리 출발), 시스템 흐름 박스들 가운데 정렬

## [2026-04-30] reflect | 빌드업 가이드 CLAUDE.md에 P4 학습 반영
- "P4+에서 도달 시 답을 정할 것" → "P5+로" 갱신
- "P4 추가에서 답이 정해진 것 (N=2)" 신규 섹션:
  - 슬롯형 setMiniBox 추상화(매핑 객체 패턴)
  - 새 부품 슬롯 디자인 (M.2 SSD 사실성)
  - 흐름 화살표 방향 (박스 외곽 출발) + 시스템 흐름 가운데 정렬
  - transition3 명명 — incremental 패턴 N=2 검증
- "P5+에서 도달 시 답을 정할 것" 신규 섹션:
  - 패널형 setMiniBox (IO Device) — 매핑 객체 흡수 vs 별도 함수
  - viewBox 공간 부족 시점 (5개 부품 시 결정)
- "현재 파일": P1+P2+P3+P4 (P5+ 미구현)
